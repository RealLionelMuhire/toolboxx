    ...
    <div className="px-2 sm:px-4 lg:px-12 py-4 md:py-8 max-w-2xl mx-auto">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <div className="space-y-1.5">
          ...
          <p className="text-xs text-gray-400">Add products or items. Use the search icon to find products from the catalog.</p>
              ...
              <div key={idx} className="flex flex-wrap gap-2 items-start p-2 border rounded bg-gray-50">
                ...
                <Input
                  ...
                  onChange={(e) => {
                    ...
                    setItems(items.map((it, i) =>
                      i === idx ? { ...cur, quantity: Math.max(1, parseInt(e.target.value, 10) || 1), unit: cur.unit || 'unit' } : it
                    ))
                  }}
                  className="w-20"
                />
                <Select
                  ...
                  onValueChange={(v) => {
                    const cur = items[idx] ?? { name: '', quantity: 1, unit: 'unit' }
                    setItems(items.map((it, i) => (i === idx ? { ...cur, unit: v } : it)))
                  }}
                >
